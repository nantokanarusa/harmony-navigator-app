...
EXPANDER_TEXTS = {
    ...
    's_t': """
        ...
        """,
    'g_t': """
        この項目は、**あなたの直感的な全体評価**です。他の細かい項目の計算結果（H）と、あなたの直感（G）がどれだけ一致しているか、あるいは**ズレているか**を知るための、非常に重要な手がかりとなります。
        
        **『計算上は良いはずなのに、なぜか気分が晴れない』**といった、貴重な自己発見のきっかけになります。
        """,
    'event_log': """
        数値だけでは分からない、**幸福度の浮き沈みの『なぜ？』**を解き明かす鍵です。
        
        **『誰と会った』『何をした』『何を感じた』**といった具体的な出来事と、グラフの変動を結びつけることで、あなたの幸福のパターンがより鮮明に見えてきます。
        """,
    'dashboard': """
...
st.title(f'🧭 Harmony Navigator (MVP v0.9.4)')
...
if st.session_state.get('username'):
    ...
    # (データ保存、ダッシュボード表示はv0.9.2と同様のロジック)
    # ...
    if submitted:
