...
EXPANDER_TEXTS = {
    'q_t': """
        #### ▼ この設定の目的
        ここでは、あなたの人生において**「何を大切にしたいか」**という価値観を数値で表現します。
        この設定が、あなたの日々の経験を評価するための**個人的な基準**となります。
        （週に一度など、定期的に見直すのがおすすめです）
        """,
    's_t': """
        #### ▼ この記録の目的
        ここでは、あなたの**実際の経験**を記録します。
        今日一日を振り返って、**実際にどう感じたか**を、各項目のスライダーで直感的に評価してください。
        この記録と、先ほど設定した価値観を比べることで、両者の間に存在する**『ズレ』**を発見できます。
        """,
    'g_t': """
        #### ▼ この項目の目的
        この項目は、**あなたの直感的な全体評価**です。
        細かいことは一度忘れて、「色々あったけど、今日の自分は全体として何点だったかな？」という感覚を表現してください。
        アプリが算出したスコア（H）と、あなたの直感（G）がどれだけ一致しているか、あるいは**ズレているか**を知るための、重要な手がかりとなります。
        """,
    'event_log': """
        #### ▼ なぜ書くのがおすすめ？
        これは、あなたの**日々の物語**を記録する場所です。
        具体的な出来事や感情を書き留めておくと、後でグラフを見たときに、数値だけでは分からない、**幸福度の変動の背景**を解き明かす鍵となります。
        """,
    'dashboard': """
        **【ダッシュボードで、何がわかるか】**

        この画面は、あなたの心の状態の**「現在地」**と**「過去の推移」**を、多角的に可視化する分析ツールです。

        ---
        
        #### **心の航海図：モデルの分析(H) vs あなたの直感(G)**
        - **これは何？**: あなたの日々の幸福度の推移です。**モデルによる計算値（H）**と、あなたの**直感的な実感（G）**を重ねて表示します。
        - **読み方**: 二つの線の乖離は、「計算上は良いはずなのに、なぜか満たされない」といった、あなた自身の自己認識のズレを発見する手がかりになります。
        ...
        - **価値観 vs 経験 レーダーチャート**:
            ...
            - **読み方**: この二つの形の**不一致**こそが、あなたの人生における構造的な**『ズレ』**です。

        - **価値観-経験 ギャップ分析 (棒グラフ)**:
            - **これは何？**: 各領域で、「価値観の構成比」から「経験の構成比」を差し引いた**『ズレの量』**を可視化したものです。
            - **読み方**:
                - **プラスの棒**: 「大切にしたいのに、経験が追いついていない」領域です。**課題や成長の機会**を示唆します。
                - **マイナスの棒**: 「さほど重視していないのに、多くの時間やエネルギーを費やしている」可能性のある領域です。

        ---

        #### **期間分析とRHI**
        - **これは何？**: あなたの幸福の**総合的なパフォーマンス**を評価します。
        - **読み方**:
            - **平均調和度 (H̄)**: この期間の、幸福の**平均点**です。
            - **RHI (リスク調整済・幸福指数)**: 平均点から、**変動と不調のリスク**を差し引いた、幸福の『持続可能性』を示す指標です。
        """
}
        """
}
