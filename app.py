...
# --- 1. 計算ロジック・インサイトエンジン関数（v1.1.0から変更なし） ---
# ... (v1.1.0のコード) ...
...
# --- 【v1.1.1新機能】ウェルカムページとガイド表示関数を、完全なコンテンツで定義 ---
def show_welcome_and_guide():
    ...
    st.markdown("""
    一言でいうと、このアプリは**あなたの「理想（こうありたい自分）」と「現実（実際に経験した一日）」の間の『ズレ』を可視化する**ための道具です。
    
    私たちの悩みやモヤモヤの多くは、この「理想」と「現実」のズレから生まれる、というのが、このアプリの背後にある理論の考え方です。
    
    このアプリを使うことで、あなたは自分だけの**「海図（チャート）」**を手に入れ、
    - **自分の現在地**（今の心の状態）を知り、
    - **目的地**（自分が本当に大切にしたいこと）を定め、
    - **航路**（日々の選択）を、自分自身で調整していくことができます。
    ...
    with st.expander("▼ 解説：クラウド上の「魔法のレストラン」"):
        st.markdown("""
        「どうして、私のPCにデータがないのに、友人たちのデータが、それぞれ別々に、安全に保存されるの？」
        
        この不思議な仕組みは、**「魔法のレストラン」**に例えることができます。
        
        - **あなた（開発者）:** このレストランの**「レシピ（`app.py`）」**を考案したシェフです。
        - **Streamlit Cloud:** そのレシピを基に、全自動で料理を提供してくれる**「魔法のレストラン（サーバー）」**です。
        - **ユーザー（あなたや友人）:** レストランにやってくる**「お客さん」**です。
        - **CSVファイル:** お客さん一人ひとりの**「プライベートな食事記録ノート」**です。
        
        お客さんが来店し、「Taroです」と名乗ると、レストランは**「Taro様専用ノート」**だけを探し出して渡します。このノートは全てレストランの中の安全な保管棚にあり、レシピを考えたシェフ（私）でさえ、勝手に見ることはできません。
        
        **結論：**
        - **あなたのデータは、私のPCには一切保存されません。**
        - あなたが入力したデータは、あなたが登録した**「船長名」だけが知っている、あなた専用の「金庫（データファイル）」**に、クラウド上で安全に保管されます。
        - **私を含め、他の誰も、あなたの個人的な記録を見ることは絶対にできません。**
...
st.title(f'🧭 Harmony Navigator (MVP v1.1.1)')
...
if st.session_state.get('username'):
    ...
    # ... (v1.1.0のメインアプリのコード、ただしエキスパンダーの中身を修正) ...
    # (データ読み込み、記録状況確認)
