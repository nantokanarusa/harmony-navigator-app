...
DEMOGRAPHIC_OPTIONS = {
    ...
    'gender_assigned_at_birth_differs': ['未選択', 'はい、異なります', 'いいえ、異なりません', '回答しない'],    
...
def show_welcome_and_guide():
    ...
    st.subheader("あなたのための、自己理解支援ツール")
    st.markdown("---")
    st.info("本アプリは、あなたの自己理解を支援するためのツールであり、幸福の実現を保証するものではありません。")
    ...
    # ★★★ ここからが新しい追加箇所 ★★★
    with st.expander("▼ このアプリが、世界的に見てもユニークな理由"):
        st.markdown("""
        既存の多くのアプリとは一線を画す、3つの独自のアプローチを統合しています。

        1.  **あなたの「価値観」が分析の中心です**
            - 他のアプリの多くは、専門家が定義した「一般的な幸福のモデル」をあなたに適用します。
            - このアプリは、**あなた自身が設定した価値観（`q_t`）**を分析の絶対的な基準とします。あなたの人生を、あなただけの「ものさし」で測る、完全にパーソナライズされたアプローチです。

        2.  **理想と現実の「ズレ」を数値化します**
            - 「なんとなく満たされない」という感覚の正体を、**価値観と経験の構造的なギャップ (`U_t`)** として、情報理論に基づき客観的に数値化します。これは、問題の根本原因を特定するための、強力な診断機能です。

        3.  **幸福の「持続可能性」を評価します**
            - 単に幸福の平均点を見るだけでなく、その**変動の激しさや、落ち込みのリスク**までを考慮に入れた、**リスク調整済・幸福指数 (`RHI`)** という先進的な指標を導入しています。これにより、あなたの幸福が長期的にも持続可能かを評価できます。
        
        この3つの組み合わせにより、他に類を見ない、深く、多角的な自己理解が可能になります。
        """)
    # ★★★ ここまでが新しい追加箇所 ★★★
    st.markdown("---")
...
def show_legal_documents():
    with st.expander("📜 **利用規約**を読む"):
        ...
        **第7条（免責事項）**
        ...
        2. 本アプリは、ユーザーの自己理解を支援するツールであり、医療行為、カウンセリング、または専門的な助言を代替するものではありません。精神的な不調を感じる場合は、必ず専門の医療機関にご相談ください。
    ...
    with st.expander("📄 **プライバシーポリシー**を読む"):
        ...
        **(2) ユーザーが記録するデータ**
        ...
        - **利用目的**:
            - 本アプリの核心機能である、幸福度の可視化（調和度H、RHI等の計算）、パターン分析、およびユーザー自身の自己理解を支援するために利用します。
...
def main():
    ...
    if auth_status in ["NOT_LOGGED_IN", "AWAITING_ID", "AWAITING_WIZARD", "AWAITING_DEMOGRAPHICS"]:
            ...
            with st.container(border=True):
                ...
                with st.expander("アプリの効果と使い方について"):
                    ...
                    **A4. いいえ、幸福になることを「保証」するものではありません。**
                    このアプリは、魔法の杖や万能薬ではありません。あくまで、あなたの自己理解を支援するための**「道具（ツール）」**です。
        ...
        with tab2:
            ...
            # 分析対象とするのは、主要な記録データ(s_t, g_t)が NaN でない行のみ
            df_analysis_data = user_data_df.dropna(subset=S_COLS + ['g_happiness'], how='all').copy()

            if df_analysis_data.empty:
                st.info('まだ記録がありません。まずは「今日の記録」タブから、最初の日誌を記録してみましょう！')
                show_sample_dashboard()
            else:
                df_processed = calculate_metrics(df_analysis_data, alpha=st.session_state.alpha_value)
                ...
                if len(df_processed) >= 7:
                    ...
                    st.info(f"現在{len(df_processed)}日分の有効なデータがあります。期間分析（RHIなど）には最低7日分のデータが必要です。")
                ...
                if not df_processed.empty:
                    ...
                    with col_chart1:
                        ...
                        fig.add_trace(go.Scatterpolar(
                              ...
                              fill='toself', name='あなたの経験 (現実の形)',
                              line=dict(color='grey'), fillcolor='rgba(128,128,128,0.3)'
                        ))
                        fig.add_trace(go.Scatterpolar(
                              r=np.append(avg_q, avg_q[0]),
                              theta=np.append(DOMAIN_NAMES_JP_VALUES, DOMAIN_NAMES_JP_VALUES[0]),
                              fill='none', name='あなたの価値観 (理想の形)',
